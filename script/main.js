...
const loadPageList = async (page) => {
    const getAnimeList = await (await fetch('/aniInfo.json')).json();
...
const loadInfoByIndex = async (index, isIndex) => {
    canClick = false;
    const infoData = (isIndex) ? (await getAnimeInfoByID(((await (await fetch('/aniInfo.json')).json()))[index].id)).Media : (await getAnimeInfoByID(index)).Media;
...
document.getElementById('inputPage').onkeydown = async (e) => {
    if (e.code === 'Enter') {
        try {
            const aniList = (await (await fetch('/aniInfo.json')).json()).length
